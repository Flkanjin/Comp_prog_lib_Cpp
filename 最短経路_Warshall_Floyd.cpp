...
bool warshallFloyd(std::vector<std::vector<long long>>& d){
    int V{int(d.size())};
    for(int k{0}; k < V; ++k) for(int i{0}; i < V; ++i) for(int j{0}; j < V; ++j) if(d[i][k] < LINF && d[k][j] < LINF) chmin(d[i][j], d[i][k] + d[k][j]);
...
void initialFromGraphList_Cost(std::vector<std::vector<edge>>& G, std::vector<std::vector<long long>>& d){
    ...
    for(int i{0}; i < V; ++i){
        for(auto& e: G[i]) d[i][e.to] = e.cost;
    }
}
