    ...
    void construct_sa(){
        ...
        for(k = 1; k <= n; k *= 2){
            auto comp_sa = [&](int i, int j){
                ...
                int ri(((i+k <= field.size()) ? this->rank[i+k] : -1));
                int rj(((j+k <= field.size()) ? this->rank[j+k] : -1));
                return ri < rj;
            };
...
public:
    ...
    std::vector<int> con_index(std::string purpose){
        ...
        int c(0), d(field.length());
        while(d-c > 1){
            int e((c+d) / 2);
            if(field.compare(sa[e], m, purpose) > 0) d = e;
            else c = e;
        }
